...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:param name="input" select="'false'"/>
	<xsl:param name="flint" select="'false'" />
	...
	<xsl:template match="result">
        ...
        <script src="/assets/vendor/modernizr.js" type="text/javascript"/>
			</head>
			<body>
					...
					<div class="wrapperContent">
						...
						<div class="main-group">
							<div class="main container" id="main_content">
								...
								<xsl:if test="$flint = 'true'">
									<div class="services-content no-related provenance-content">
               ...
               <div class="main container">
                  <div id="footer-pane-first" class="footer-pane">
                     <ul class="social-icons">
                        <li><a class="btn-social btn-social-facebook" target="_blank" href="https://www.facebook.com/officialgazette" title="The Gazette on Facebook"><span class="accessible-text">The Gazette on Facebook</span></a></li>
                        <li><a class="btn-social btn-social-linkedin" target="_blank" href="http://www.linkedin.com/company/the-official-gazette?trk=company_logo" title="The Gazette on LinkedIn"><span class="accessible-text">The Gazette on LinkedIn"</span></a></li>
                        <li><a class="btn-social btn-social-twitter" target="_blank" href="https://twitter.com/TheGazetteUK" title="The Gazette on Twitter"><span class="accessible-text">@TheGazetteUK</span></a></li>
					 </ul><a href="http://www.tsoshop.co.uk/The-Gazette" target="_blank"><img src="/assets/chrome/images/tso.png?build=1.59-SNAPSHOT" alt="TSO" class="logo" /></a></div>
                  <nav role="navigation">
                     <div class="nav-container footer-nav">
                        <ul class="footer-lists">
                          <xsl:copy-of select="/result/footer/*" />
                        </ul>
                     </div>
                  </nav>
                  <p class="help-tip help-tip-content published-by-authority" id="published-by-authority"> The Gazette is published by TSO (The Stationery Office) under the superintendence
                     of Her Majesty's Stationery Office (HMSO), part of <a href="http://www.nationalarchives.gov.uk/" target="_blank">The National Archives</a></p>
               </div>
            </div>
            <div class="footer-messages">
               <div class="main container">
                  <div class="open-government-licence">
                     <h2><a href="http://www.nationalarchives.gov.uk/doc/open-government-licence/version/3" target="_blank">Open Government Licence</a></h2>
                     <p>All content is available under the <a href="http://www.nationalarchives.gov.uk/doc/open-government-licence/version/3" target="_blank">Open Government Licence v3.0</a>, except where otherwise stated. However, please note that this
				...
				<script data-main="/assets/init" src="/assets/vendor/require.js/require-jquery.js"/>
				<script type="text/javascript">

					<!--
                        var _gaq = _gaq ||[];
                        _gaq.push([ '_setAccount', 'UA-39464953-4']);
				...
				<script>
					$("select[name=method]", "form.pagePreview").on("change", function()
					{
						$("input[type=file], textarea").hide();
						$("*[name=" + $(this).val() + "]", "form.pagePreview").show();
					});
					$("select[name=method]", "form.pagePreview").trigger("change");
				</script>
	...
	<xsl:template match="result" mode="title">
		<title>
			<xsl:choose>
				<xsl:when test="primaryTopic/label">
	...
	<xsl:template match="result" mode="style">
		<link rel="stylesheet" href="/assets/chrome/all-notices.css" type="text/css"/>
		...
		<link rel="stylesheet" href="/scripts/sparql/lib/codemirror.css" type="text/css"/>
		<link rel="stylesheet" href="/scripts/sparql/css/sparqlcolors.css" type="text/css"/>
		<link rel="stylesheet" href="/scripts/sparql/css/docs.css" type="text/css"/>
		...
		<xsl:apply-templates select="site/stylesheet"/>
	</xsl:template>
	...
	<xsl:template match="result" mode="script">
		...
		<xsl:if test="$showMap = 'true'">
			<script type="text/javascript" src="http://openspace.ordnancesurvey.co.uk/osmapapi/openspace.js?key={$openSpaceAPIkey}"/>
		</xsl:if>
		<xsl:if test="$flint='true'">
			<script type="text/javascript" src="/scripts/sparql/init-lda.js">//</script>
			<script type="text/javascript" src="/scripts/sparql/flint-editor.js">//</script>
		</xsl:if>
		<!--link rel="stylesheet" href="/scripts/sparql/lib/codemirror.css"/>
	<link rel="stylesheet" href="/scripts/sparql/css/sparqlcolors.css"/>
	<link rel="stylesheet" href="/scripts/sparql/css/docs.css"/-->
	...
	<script type="text/javascript" src="/scripts/codemirror/sparql.js"></script-->

		<script type="text/javascript">
		$(document).ready(function() {

			<!--   /* $("tr.description td.value").shorten(
        { width: 100, tail: ' <a class="more" href="#">...</a>', tooltip: false }
        );
			$('.info img')
				.toggle(function () {
					$(this)
...
-->
	...
	<xsl:template match="result" mode="showMap">
		<xsl:param name="items"
			select="items/item[position() &lt;= 10]/descendant-or-self::*[*] | primaryTopic[not(../items)]/descendant-or-self::*[*]"/>
		<xsl:variable name="easting" select="key('propertyTerms', $easting-uri)/label"/>
		<xsl:choose>
	...
	<xsl:template match="result" mode="header">
				...
				<div class="main container">
					<div class="nav-container navbar">
						<div class="content">
							...
							<xsl:copy-of select="/result/header/*" />
							</ul>
							<!--<ul class="account nav">
                  <li><a href="/my-gazette/account?register=true" class="my-gazettes-register">Register</a></li>
		...
		<div class="header-group" role="heading" aria-level="1">
			<div class="main container">
				<header role="banner">
					<div class="site-header">
						<h1 class="accessible-text">The Gazette</h1><a href="/" title="The Gazette Home Page" class="site-logo"><img src="/assets/chrome/images/logo.png?build=default" alt="The Gazette" /></a>
						<p class="strapline"><a data-tooltip="middle:top" href="#published-by-authority" title="Published by Authority">Published by Authority</a> | Est 1665</p>
					</div>
				</header>
			</div>
		</div>
		...
		<div class="service-navigation-group">
			<div class="main container">
				<xsl:choose>
					<xsl:when test="count(.//format/item) &gt; 0" >
						<div class="nav-container service-nav">
							<xsl:apply-templates select="." mode="formats"/>
						</div>
					</xsl:when>
					<xsl:otherwise>
						<div class="nav-container no-service-nav"></div>
					</xsl:otherwise>
				</xsl:choose>
			</div>
		</div>
	...
	<xsl:template match="result" mode="footer">
		...
		<div class="main-pane">
			...
			<xsl:apply-templates select="site/javascript"/>
		</div>
	</xsl:template>
	...
	<!-- SNC reckless mod replacing query template to use Flint -->
	<xsl:template match="query">
		<div id="query" style="display:none;">
			<xsl:value-of select="."/>
		</div>
		<!--script type="text/javascript">var globalQuery="<xsl:value-of select='encode-for-uri(.)'/>";</script-->
	</xsl:template>
...
<xsl:template match="query" mode="skipLines">
	...
	<xsl:template match="result" mode="formats">
		<ul class="nav nav-tabs" id="navigation">
			<xsl:for-each select="format/item">
				<li class="format-{name}">
					<xsl:if test="position() = 1">
						<xsl:attribute name="class">first format-<xsl:value-of select="name"
							/></xsl:attribute>
					</xsl:if>
					<xsl:if test="position() = last()">
						<xsl:attribute name="class">last format-<xsl:value-of select="name"
							/></xsl:attribute>
					</xsl:if>
	...
	<xsl:template match="result" mode="content" priority="10">
		...
		<div class="main-pane" id="result">
			<article class="news-article linked-data">
				<xsl:choose>
					<xsl:when test="items">
						<header>
							<!--
						<xsl:if test="items/item">
							...
							<h1 class="title">
								<xsl:if test="not(label)"> Search Results </xsl:if>
								<xsl:value-of select="label"/>
							</h1>
						</header>
						<xsl:if test="items/item">
	...
	<xsl:template match="result" mode="topnav">
		...
		<xsl:variable name="hasSiteLinks" select="site/link"/>
		<nav class="topnav">
			<xsl:if test="$hasSiteLinks">
				<xsl:apply-templates select="site" mode="apiTopNav"/>
			</xsl:if>
		...
		<xsl:choose>
			<xsl:when test="$showMap = 'true'">
				<xsl:variable name="markers"
					select="items//*[*[name(.) = $easting] and *[name(.) = $northing]] | primaryTopic[not(../items)]/descendant-or-self::*[*[name(.) = $easting] and *[name(.) = $northing]]"/>
	...
	<xsl:template match="result" mode="graphs">
		...
		<xsl:if test="$rows != ''">
			<section class="graphs">
				<h1>Graphs</h1>
				<xsl:call-template name="createInfo">
					<xsl:with-param name="text">These graphs summarise the values of the numeric
						properties of these items.</xsl:with-param>
				</xsl:call-template>
				<table>
	...
	<xsl:template match="*" mode="graphRow">
		...
		<xsl:choose>
			...
			<xsl:when
				test="@datatype = 'integer' or @datatype = 'decimal' or @datatype = 'int' or @datatype = 'float' or @datatype = 'double'">
	...
	<xsl:template match="*" mode="showBarchart">
		...
		<xsl:choose>
			<xsl:when
				test="name(.) = $easting or name(.) = $northing or name(.) = $lat or name(.) = $long"/>
			<xsl:otherwise>
	...
	<xsl:template match="result" mode="summary">
		<xsl:if test="count(items/item) > 1">
			<div class="block block-lda">
				<header>
					<h3 class="title">On This Page</h3>
				<!--xsl:call-template name="createInfo">
					<xsl:with-param name="text">Links to the items within this page, and to the
						previous and/or next pages of results.</xsl:with-param>
				</xsl:call-template-->
				</header>
				<div class="content">
					<ul>
	...
	<xsl:template match="site" mode="apiTopNav">
		<!--<section>
			<h1>Browse</h1>
			<xsl:apply-templates select="link" mode="apiTopNav"/>
		</section>-->
	</xsl:template>

	<xsl:template match="link" mode="apiTopNav">
		<ul>
			<xsl:apply-templates select="item" mode="apiTopNav"/>
		</ul>
	</xsl:template>

	<xsl:template match="link/item" mode="apiTopNav">
		<li>
			<a>
				<xsl:attribute name="href">
					<xsl:value-of select="@href"/>
				...
				<xsl:choose>
					<xsl:when test="$label/item">
						<xsl:for-each select="$label/item[not(. = preceding-sibling::item)]">
							<xsl:sort select="@xml:lang = 'en-gb'" order="descending"/>
	...
	<xsl:template match="result" mode="filternav">
		...
		<xsl:if test="$filters != ''">
			<div class="block block-lda">
				...
				<div class="content">
				<!--xsl:call-template name="createInfo">
					<xsl:with-param name="text">These are the filters currently being used to limit
						the search results. Click on the <img src="{$activeImageBase}/Back.png"
							alt="remove filter"/> icon to remove the filter.</xsl:with-param>
				</xsl:call-template-->
				<table>
	...
	<xsl:template name="extractFilters">
		...
		<xsl:if
			test="not(starts-with($param, '_')) and not(starts-with($param, 'view')) and not(starts-with($param, concat('max-', $easting, '=')) or starts-with($param, concat('min-', $northing, '=')) or starts-with($param, concat('max-', $northing, '=')))">
						...
						<th class="label">
							<xsl:choose>
								<xsl:when test="$isLabelParam = 'true'">
									<xsl:value-of select="translate($paramName,'_',' ')"/>
								</xsl:when>
								<xsl:when test="$paramName = concat('min-', $easting)">area on
									map</xsl:when>
								<xsl:otherwise>
	...
	<xsl:template match="result" mode="viewnav">
		...
		<xsl:if test="count(/.//format/item) &gt; 0">
			<aside class="view">
				<div class="block block-lda">
					<header>
						...
						<!--xsl:call-template name="createInfo">
							<xsl:with-param name="text">
								<p>Choose what information you want to view about each item. There are
									some pre-defined views, but starred properties are always present no
									matter what the view. </p>
								<p>You can star properties by clicking on the <img
										src="{$inactiveImageBase}/Star.png" alt="star this property"/>
									icon. The currently starred icons have a <img
										src="{$activeImageBase}/Star.png" alt="unstar this property"/>
									icon; clicking on it will unstar the property.</p>
							</xsl:with-param>
						</xsl:call-template-->
					</header>
					<div class="content">
						...
						<br />
						<p><a href="/data">For further information please visit our Data page</a></p>
					</div>
					<!--
			<ul class="properties">
				<xsl:if test="$properties != ''">
		...
		<xsl:if test="contains($properties, ',')">
			<xsl:apply-templates select="." mode="selectedProperties">
				...
				<xsl:with-param name="previousProperties"
					select="concat($previousProperties, ',', $property)"/>
			</xsl:apply-templates>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template match="result" mode="sizenav">
		<div class="block block-lda">
		<header>
			<h3 class="title">Items per page</h3>
		</header>
			<!--xsl:call-template name="createInfo">
				<xsl:with-param name="text">Choose how many items to view on each page. The more
					items you view, the longer the page will take to load.</xsl:with-param>
			</xsl:call-template-->
			<div class="content">
			<ul>
				<li>
				...
				<xsl:choose>
					<xsl:when test="$orderBy != ''">
						<xsl:variable name="description">
							<xsl:choose>
								<xsl:when
									test="starts-with($orderBy, concat('(((?', $easting, ' - ')) or starts-with($orderBy, concat('desc(((?', $easting, ' - '))"
									> proximity to centre of map</xsl:when>
							...
							<xsl:choose>
								...
								<xsl:when test="starts-with($orderBy, 'desc')">
									<a rel="nofollow" title="sort in ascending order">
										...
										<img src="{$activeImageBase}/Arrow3 Down.png"
								...
								<xsl:when test="starts-with($orderBy, 'asc')">
									<a rel="nofollow" title="sort in descending order">
										...
										<img src="{$activeImageBase}/Arrow3 Up.png"
	...
	<xsl:template match="*" mode="sort">
		...
		<xsl:if
			test="not(contains(concat(',', $current, ','), concat(',', $name, ',')) or contains(concat(',', $current, ','), concat(',-', $name, ',')))">
	...
	<xsl:template match="*" mode="paramHierarchy">
		<xsl:if
			test="not(parent::item/parent::items/parent::result or parent::primaryTopic/parent::result)">
	...
	<xsl:template match="format/item" mode="nav">
		...
		<a href="{@href}" class="format-{$name}" type="{format/label}" rel="alternate"
			title="view in {$name} format">
			<xsl:attribute name="class">
				<xsl:text>format-</xsl:text>
				<xsl:value-of select="$name"/>
				<xsl:if test="$name='html'">
	...
	<xsl:template match="/result/primaryTopic" mode="content" priority="10">
		<header>
			...
			<p class="id">
				<input size="50" type="url" readonly="readonly">
	...
	<xsl:template match="items/item" mode="header"> </xsl:template>
						...
						<caption>
							<xsl:apply-templates select="." mode="link">
								<xsl:with-param name="content">
									<xsl:choose>
										<xsl:when
											test="self::item/parent::items/parent::result or starts-with(@href, ancestor::*[@href][1]/@href)">
											<xsl:call-template name="lastURIpart">
												<xsl:with-param name="uri" select="@href"/>
											</xsl:call-template>
										</xsl:when>
	...
	<xsl:template match="*" mode="row">
		...
		<xsl:if test="name() != $bestLabelParam">
			...
			<tr class="{name(.)}-row">
				<xsl:if test="$last">
					<xsl:attribute name="class"><xsl:value-of select="name(.)"/>
						last</xsl:attribute>
				</xsl:if>
				<xsl:if test="$properties != ''">
					<td class="select">
				...
				<xsl:choose>
					...
					<xsl:when
						test="(name(.) = $northing or name(.) = $lat or name(.) = $long) and $showMap = 'true'">
						<td class="value">
							<xsl:apply-templates select="." mode="display"/>
						</td>
					</xsl:when>
	...
	<xsl:template name="createInfo">
		...
		<div class="info help">
			<a href="#help-with-lda" data-tooltip="left:middle" class="open ">
				<b>?</b>
			</a>
			<div class="help-tip help-tip-content" id="help-with-lda">
	...
	<xsl:template name="splitPath">
		...
		<xsl:choose>
			<xsl:when test="contains($paramName, '_')">
				<xsl:value-of select="substring-before($paramName, '_')"/>
				<xsl:text> </xsl:text>
				<xsl:call-template name="splitPath">
					<xsl:with-param name="paramName" select="substring-after($paramName, '_')"/>
	...
	<xsl:template
		match="*[@datatype = 'integer' or @datatype = 'decimal' or @datatype = 'int' or @datatype = 'float' or @datatype = 'double']"
		mode="display">
		<xsl:apply-templates select="." mode="content"/>
		<xsl:if test="not(/result/next) and not(self::item)">
			...
			<xsl:if test="$showBoxplot = 'true'">
				<span class="boxplot" id="boxplot{generate-id()}"> </span>
				<script type="text/javascript">
	...
	<xsl:template match="*" mode="showBoxplot">
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				...
				<xsl:value-of
					select="$distinctValues != 1 and count($values) > 5 and not($values/item)"/>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	...
	<xsl:template match="*[item]" mode="content" priority="4">
		...
		<xsl:choose>
			<xsl:when
				test="$anyItemHasNonLabelProperties = 'true' and $anyItemIsHighestDescription = 'true'">
	...
	<xsl:template match="*[*]" mode="content" priority="3">
		...
		<xsl:choose>
			<xsl:when
				test="$nested or ($hasNonLabelProperties = 'true' and $isHighestDescription = 'true')">
				<xsl:apply-templates select="." mode="table"/>
			</xsl:when>
			<xsl:otherwise>
				<xsl:apply-templates select="." mode="link">
		...
		<xsl:choose>
			<xsl:when test="contains($normalisedProperties, $entry)">
				...
				<xsl:variable name="value">
					...
					<xsl:if
						test="not($before = ',' or $before = '') and not($after = ',' or $after = '')"
						>,</xsl:if>
	...
	<xsl:template match="*" mode="filter">
		<!-- Magnifying glass -->
		<!-- 2886 and false() added to prevent display of < > -->
		<!--
		<xsl:param name="paramName">
			<xsl:apply-templates select="." mode="paramName"/>
		</xsl:param>
		...
		<xsl:variable name="lang" select=".//@xml:lang[1]"/>
		<xsl:choose>
			<xsl:when test="$value = ''"/>
			...
			<xsl:when
				test="($datatype = 'integer' or $datatype = 'decimal' or $datatype = 'float' or $datatype = 'int' or $datatype = 'date' or $datatype = 'dateTime' or $datatype = 'time') and false()">
				...
				<xsl:choose>
					...
					<xsl:otherwise>
						<a rel="nofollow" title="filter to values less than {$value}">
							...
							<xsl:choose>
								<xsl:when test="$max != ''">
									<img src="{$activeImageBase}/Arrow3 Left.png"
										alt="less than {$value}"/>
								</xsl:when>
								<xsl:otherwise>
									<img src="{$inactiveImageBase}/Arrow3 Left.png"
										alt="less than {$value}"/>
								</xsl:otherwise>
							</xsl:choose>
						</a>
				...
				<a rel="nofollow" title="more like this">
					<xsl:attribute name="href">
						...
						<xsl:value-of select="concat('&amp;_lang=',$lang)"/>
						<xsl:text>&amp;view=linked-data</xsl:text>
					</xsl:attribute>
				...
				<xsl:choose>
					...
					<xsl:otherwise>
						<a rel="nofollow" title="more than {$value}">
							...
							<xsl:choose>
								<xsl:when test="$min != ''">
									<img src="{$activeImageBase}/Arrow3 Right.png"
										alt="more than {$value}"/>
								</xsl:when>
								<xsl:otherwise>
									<img src="{$inactiveImageBase}/Arrow3 Right.png"
										alt="more than {$value}"/>
								</xsl:otherwise>
							</xsl:choose>
						</a>
						...
						<xsl:choose>
							...
							<xsl:otherwise>
								...
								<xsl:if test="$adjustedHref != @href">
									<xsl:text> </xsl:text>
									<a href="{@href}" title="view original" class="outlink"
										>original</a>
								</xsl:if>
							</xsl:otherwise>
						</xsl:choose>
			...
			<xsl:if test="items/item[@href] or (not(items) and primaryTopic)">
				<form action="{$searchURI}">
					...
					<table>
						...
						<xsl:for-each
							select="(items/item/* | primaryTopic/*)[generate-id(key('properties', name(.))[1]) = generate-id(.)]">
							<xsl:sort select="name(.) = $prefLabel" order="descending"/>
							<xsl:sort select="name(.) = $name" order="descending"/>
							<xsl:sort select="name(.) = $title" order="descending"/>
							<xsl:sort select="name(.) = $label" order="descending"/>
	...
	<xsl:template match="*" mode="input">
		...
		<xsl:choose>
			...
			<xsl:when
				test="@datatype = 'integer' or @datatype = 'decimal' or @datatype = 'float' or @datatype = 'double' or @datatype = 'int' or @datatype = 'date' or @datatype = 'dateTime' or @datatype = 'time'">
				<xsl:variable name="min">
					<xsl:call-template name="paramValue">
						<xsl:with-param name="uri">
							<xsl:apply-templates select="/result" mode="searchURI"/>
		...
		<xsl:choose>
			...
			<xsl:when test="@datatype = 'dateTime'">
				...
				<xsl:attribute name="pattern"
					>[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}</xsl:attribute>
			</xsl:when>
			<xsl:when
				test="@datatype = 'integer' or @datatype = 'decimal' or @datatype = 'float' or @datatype = 'double' or @datatype = 'int'">
	...
	<xsl:template name="form">
		<div class="main-pane" id="form">
			<article class="news-article linked-data">
				<header>
					<h1 class="title">Preview correction</h1>
				</header>
				<section>
					<div class="content">
						<form action="/services/gazettePagePreview" method="post"
							class="pagePreview" enctype="multipart/form-data">
							<p>Choose how do you want to send us your file:</p>
							<select name="method">
								<option value="file">File upload</option>
								<option value="textarea">Direct input</option>
							</select>
