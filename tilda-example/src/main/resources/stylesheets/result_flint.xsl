...
<xsl:template match="stylesheet">
    <link rel="stylesheet" type="text/css"><xsl:attribute name="href"><xsl:value-of select="@href"/></xsl:attribute></link>
</xsl:template>
...
<xsl:template match="result" mode="script">
	...
	<script type="text/javascript" src="/scripts/sparql/init-local.js">//</script>
...
<xsl:template match="result" mode="header">
    ...
    <h1>
      <a href="/">
        <xsl:choose>
          <xsl:when test="site/label"><xsl:value-of select="/result/site/label"/></xsl:when>
            <xsl:otherwise>Linked Data API</xsl:otherwise>
      </xsl:choose>
      </a>
    </h1>
	</header>
...
<xsl:template match="viewingResult | selectionResult" mode="footer">
	...
	<div id="flint-editor"></div>
</xsl:template>

<!-- SNC reckless mod replacing query template to use Flint -->
<xsl:template match="query">
  <div id="query" style="display:none;"><xsl:value-of select="."/></div>
...
<xsl:template match="result" mode="formats">
	<section class="formats">
        ...
        <li class="format-{name}">
            ...
            <xsl:attribute name="class">first format-<xsl:value-of select="name"/></xsl:attribute>
					</xsl:if>
					<xsl:if test="position() = last()">
            <xsl:attribute name="class">last format-<xsl:value-of select="name"/></xsl:attribute>
					</xsl:if>
...
<xsl:template match="link/item" mode="apiTopNav">
  <li>
    <a><xsl:attribute name="href"><xsl:value-of select="@href"/></xsl:attribute><xsl:value-of select="label"/></a>
    <xsl:apply-templates select="link" mode="apiTopNav"/>
  </li>
</xsl:template>
...
<xsl:template name="extractFilters">
	...
	<xsl:if test="not(starts-with($param, '_')) and not(starts-with($param, 'view')) and not(starts-with($param, concat('max-', $easting, '=')) or starts-with($param, concat('min-', $northing, '=')) or starts-with($param, concat('max-', $northing, '=')))">
...
<xsl:template match="format/item" mode="nav">
  ...
  <a href="{@href}" class="format-{name}" type="{format/label}" rel="alternate" title="view in {$name} format">
		<xsl:value-of select="label" />
	</a>
</xsl:template>
...
<xsl:template match="/result/primaryTopic" mode="content" priority="10">
    ...
    <p class="id"><input size="50" type="url" readonly="readonly"><xsl:attribute name="value"><xsl:value-of select="@href" /></xsl:attribute></input></p>
	</header>
